...
def price_tracker(product_name):
    ...
    def price_comparison(product_name):
        ...
        for table in daily_tables:
            ...
            if not df_daily.empty:
                df_daily["date"] = pd.to_datetime(
                    df_daily["date"].str.replace("products_", "", regex=True),
                    format="%Y_%m_%d",
                )
        ...
        # Daily Prices
        plt.plot(
            df_daily_combined["date"],
            df_daily_combined["price"],
            marker="o",
            linestyle="-",
        ...
        print(
            f"Price comparison chart saved as 'price_comparison_{product_name}.png'\n"
        )
    ...
    insert_products(aldi(product_name), "Aldi")
